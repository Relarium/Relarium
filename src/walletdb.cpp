...
bool CWalletDB::WritePurpose(const string& strAddress, const string& sRLMurpose)
{
    nWalletDBUpdated++;
    return Write(make_pair(string("purpose"), strAddress), sRLMurpose);
}

bool CWalletDB::ErasePurpose(const string& sRLMurpose)
{
    nWalletDBUpdated++;
    return Erase(make_pair(string("purpose"), sRLMurpose));
}
        ...
        if (strType == "name") {
            ...
            if (31404 <= wtx.fTimeReceivedIsTxTime && wtx.fTimeReceivedIsTxTime <= 31703) {
                if (!ssValue.empty()) {
                    ...
                    strErr = sRLMrintf("LoadWallet() upgrading tx ver=%d %d '%s' %s",
                        wtx.fTimeReceivedIsTxTime, fTmp, wtx.strFromAccount, hash.ToString());
                    wtx.fTimeReceivedIsTxTime = fTmp;
                } else {
                    strErr = sRLMrintf("LoadWallet() repairing tx ver=%d %s", wtx.fTimeReceivedIsTxTime, hash.ToString());
            ...
            if (pwallet->mapMasterKeys.count(nID) != 0) {
                strErr = sRLMrintf("Error reading wallet database: duplicate CMasterKey id %u", nID);
                return false;
            }
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    // found.
    int64_t now = GetTime();
    std::string newFilename = sRLMrintf("wallet.%d.bak", now);
