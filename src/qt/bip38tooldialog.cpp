...
bool isValidPassphrase(QString sRLMassphrase, QString& strInvalid)
{
    for (int i = 0; i < sRLMassphrase.size(); i++) {
        if (!validChar.contains(sRLMassphrase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(sRLMassphrase[i]))
                continue;

            strInvalid = sRLMassphrase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qsRLMassphrase = ui->passphraseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassphrase(qsRLMassphrase, strInvalid)) {
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qsRLMassphrase.toStdString(), key.GetPrivKey_256());
...
{
    string sRLMassphrase = ui->passphraseIn_DEC->text().toStdString();
