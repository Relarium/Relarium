...
#if defined(NDEBUG)
#error "Relarium cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 3 * COIN;
    } else if (nHeight <= 99999999999 && nHeight > 3500) {
        nSubsidy = 30 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        if (check())
                            return state.Invalid(false, REJECT_NONSTANDARD, sRLMrintf("non-mandatory-script-verify-flag (%s)", ScriptErrorString(check.GetScriptError())));
                    }
                    ...
                    return state.DoS(100, false, REJECT_INVALID, sRLMrintf("mandatory-script-verify-flag-failed (%s)", ScriptErrorString(check.GetScriptError())));
                }
            }
        }
    }
...
{
    RenameThread("relarium-scriptch");
...
{
    return GetDataDir() / "blocks" / sRLMrintf("%s%05u.dat", prefix, pos.nFile);
}
...
bool static LoadBlockIndexDB()
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
        ...
        if (!ReadBlockFromDisk(lastMetaBlock, pindexLastMeta)) {
            isFixed = false;
            strError = sRLMrintf("failed to read block %d from disk", pindexLastMeta->nHeight);
        }
        ...
        if (!ReadBlockFromDisk(lastBlock, pindexLast)) {
            isFixed = false;
            strError = sRLMrintf("failed to read block %d from disk", pindexLast->nHeight);
        }
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
                sRLMrintf("Version must be %d or greater", ActiveProtocol()));
...
{
    return sRLMrintf("CBlockFileInfo(blocks=%u, size=%u, heights=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst, nHeightLast, DateTimeStrFormat("%Y-%m-%d", nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
}
