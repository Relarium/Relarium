...
//! These are created by StarRLMCThreads, destroyed in StopRPCThreads
    ...
    bool fAllowNull)
{
    ...
    BOOST_FOREACH (Value_type t, typesExpected) {
        ...
        if (!((v.type() == t) || (fAllowNull && (v.type() == null_type)))) {
            string err = sRLMrintf("Expected type %s, got %s",
    ...
    bool fAllowNull)
{
    BOOST_FOREACH (const PAIRTYPE(string, Value_type) & t, typesExpected) {
        ...
        if (!fAllowNull && v.type() == null_type)
            throw JSONRPCError(RPC_TYPE_ERROR, sRLMrintf("Missing %s", t.first));

        if (!((v.type() == t.second) || (fAllowNull && (v.type() == null_type)))) {
            string err = sRLMrintf("Expected type %s for %s, got %s",
...
string CRPCTable::help(string strCommand) const
{
    ...
    if (strRet == "")
        strRet = sRLMrintf("help: unknown command: %s\n", strCommand);
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Relarium server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Relarium server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* Relarium features */
        {"relarium", "masternode", &masternode, true, true, false},
        {"relarium", "masternodelist", &masternodelist, true, true, false},
        {"relarium", "mnbudget", &mnbudget, true, true, false},
        {"relarium", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"relarium", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"relarium", "mnsync", &mnsync, true, true, false},
        {"relarium", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"relarium", "obfuscation", &obfuscation, false, false, true}, /* not threadSafe because of SendMoney */
...
void StarRLMCThreads()
{
        ...
        BOOST_FOREACH (string strAllow, vAllow) {
            ...
            if (!subnet.IsValid()) {
                uiInterface.ThreadSafeMessageBox(
                    sRLMrintf("Invalid -rpcallowip subnet specification: %s. Valid are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24).", strAllow),
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(sRLMrintf(
                                             _("To use relariumd, or the -server option to relarium-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
    ...
    } else if (mapArgs.count("-rpcbind")) // Specific bind address
    {
        BOOST_FOREACH (const std::string& addr, mapMultiArgs["-rpcbind"]) {
            try {
                ...
                uiInterface.ThreadSafeMessageBox(
                    sRLMrintf(_("Could not parse -rpcbind value %s as network address"), addr),
                    "", CClientUIInterface::MSG_ERROR);
                StartShutdown();
                return;
            }
    ...
    BOOST_FOREACH (const ip::tcp::endpoint& endpoint, vEndpoints) {
        try {
            ...
            strerr = sRLMrintf(_("An error occurred while setting up the RPC address %s port %u for listening: %s"), straddress, endpoint.port(), e.what());
        }
    }
...
void SeRLMCWarmupStatus(const std::string& newStatus)
{
    LOCK(cs_rpcWarmup);
    rpcWarmupStatus = newStatus;
}

void SeRLMCWarmupFinished()
{
    LOCK(cs_rpcWarmup);
    assert(fRPCInWarmup);
    fRPCInWarmup = false;
...
{
    return "> relarium-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:24621/\n";
}
