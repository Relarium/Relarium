...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total relarium balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized relarium balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in relarium/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in relarium/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"relariumaddress\"\n"
            "\nReturn information about the given relarium address.\n"
            "\nArguments:\n"
            "1. \"relariumaddress\"     (string, required) The relarium address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"relariumaddress\", (string) The relarium address validated\n"
...
CScript _createmultisig_redeemScript(const Array& params)
{
    ...
    if ((int)keys.size() < nRequired)
        throw runtime_error(
            sRLMrintf("not enough keys supplied "
...
#ifdef ENABLE_WALLET
        // Case 1: Relarium address and we have full public key:
        CBitcoinAddress address(ks);
        if (pwalletMain && address.IsValid()) {
            CKeyID keyID;
            if (!address.GetKeyID(keyID))
                throw runtime_error(
                    sRLMrintf("%s does not refer to a key", ks));
            CPubKey vchPubKey;
            if (!pwalletMain->GetPubKey(keyID, vchPubKey))
                throw runtime_error(
                    sRLMrintf("no full public key for address %s", ks));
    ...
    if (result.size() > MAX_SCRIPT_ELEMENT_SIZE)
        throw runtime_error(
            sRLMrintf("redeemScript exceeds size limit: %d > %d", result.size(), MAX_SCRIPT_ELEMENT_SIZE));
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are relarium addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) relarium address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"relariumaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"relariumaddress\"  (string, required) The relarium address to use for the signature.\n"
