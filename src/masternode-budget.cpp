...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    ...
    if (!GetTransaction(nTxCollateralHash, txCollateral, nBlockHash, true)) {
        strError = sRLMrintf("Can't find collateral tx %s", txCollateral.ToString());
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
            strError = sRLMrintf("Invalid Script %s", txCollateral.ToString());
            LogPrintf("CBudgetProposalBroadcast::IsBudgetCollateralValid - %s\n", strError);
            return false;
        }
    ...
    if (!foundOpReturn) {
        strError = sRLMrintf("Couldn't find opReturn %s in %s", nExpectedHash.ToString(), txCollateral.ToString());
        LogPrintf("CBudgetProposalBroadcast::IsBudgetCollateralValid - %s\n", strError);
        return false;
    }
    ...
    */
    ...
    if (conf >= BUDGET_FEE_CONFIRMATIONS) {
        ...
        strError = sRLMrintf("Collateral requires at least %d confirmations - %d confirmations", BUDGET_FEE_CONFIRMATIONS, conf);
...
CBudgetProposal* CBudgetManager::FindProposal(const std::string& sRLMroposalName)
{
    ...
    while (it != mapProposals.end()) {
        if ((*it).second.sRLMroposalName == sRLMroposalName && (*it).second.GetYeas() > nYesCount) {
...
CBudgetProposal::CBudgetProposal(std::string sRLMroposalNameIn, std::string strURLIn, int nBlockStartIn, int nBlockEndIn, CScript addressIn, CAmount nAmountIn, uint256 nFeeTXHashIn)
{
    sRLMroposalName = sRLMroposalNameIn;
    strURL = strURLIn;
...
{
    sRLMroposalName = other.sRLMroposalName;
    strURL = other.strURL;
...
bool CBudgetProposal::AddOrUpdateVote(CBudgetVote& vote, std::string& strError)
{
    ...
    if (vote.nTime > GetTime() + (60 * 60)) {
        strError = sRLMrintf("new vote is too far ahead of current time - %s - nTime %lli - Max Time %lli\n", vote.GetHash().ToString(), vote.nTime, GetTime() + (60 * 60));
        LogPrint("mnbudget", "CBudgetProposal::AddOrUpdateVote - %s\n", strError);
        return false;
    }
...
CBudgetProposalBroadcast::CBudgetProposalBroadcast(std::string sRLMroposalNameIn, std::string strURLIn, int nPaymentCount, CScript addressIn, CAmount nAmountIn, int nBlockStartIn, uint256 nFeeTXHashIn)
{
    sRLMroposalName = sRLMroposalNameIn;
    strURL = strURLIn;
