...
// Allocated in IniRLMCMining, free'd in ShutdownRPCMining
...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or relarium.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Relarium is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Relarium is downloading blocks...");
...
// Release the wallet and main lock while waiting
    ...
    result.push_back(Pair("bits", sRLMrintf("%08x", pblock->nBits)));
